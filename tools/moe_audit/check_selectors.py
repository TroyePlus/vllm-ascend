...
namespace = dict(AscendDeviceType=soc_type, MoECommType=comm_type,
                 get_ep_group=lambda: NS(world_size=8), is_moe_model=lambda _: True,
                 get_mc2_tokens_capacity=lambda: 32,
                 get_ascend_config=lambda: NS(enable_fused_mc2=0),
                 logger=NS(debug=lambda *a, **k: None))
exec(compile(module, str(source), "exec"), namespace)
config = NS(parallel_config=NS(enable_expert_parallel=True, world_size_across_dp=8,
                               pipeline_parallel_size=1),
            model_config=NS(get_num_experts=lambda: 256,
                            hf_text_config=NS(n_routed_experts=256, moe_quantize="w8a8_dynamic")))
for soc in (soc_type.A2, soc_type.A3):
    namespace["get_ascend_device_type"] = lambda: soc
