...
def _emit(event, **fields):
    print("[MOE_AUDIT] " + json.dumps(dict(event=event, pid=os.getpid(), **fields),
...
def _metadata(metadata):
    ...
    return {key: _plain(getattr(metadata, key, None)) for key in
            ("num_prefills", "num_prefill_tokens", "num_decode_tokens", "num_actual_tokens")}
...
def audit_forward(config, ctx, selected_tokens, actual_tokens, metadata, skip_compiled):
    ...
    if not state["config_printed"]:
        ...
        _emit("CONFIG", **ranks, dp_size=dp.world_size, tp_size=tp.world_size, ep_size=ep.world_size,
              soc=str(get_ascend_device_type()), ep_enabled=pc.enable_expert_parallel,
              ...
              prefill_mc2=ac.enable_prefill_mc2, fused_mc2=ac.enable_fused_mc2,
              capacity=get_mc2_tokens_capacity(), max_batch=sc.max_num_batched_tokens,
              max_seqs=sc.max_num_seqs, capture_max=cc.max_cudagraph_capture_size,
              eager=config.model_config.enforce_eager, compile_mode=str(cc.mode), backend=str(cc.backend),
              ...
              force_alltoall=os.getenv("DSV4_TEST_FORCE_ALLTOALL"), force_mc2=os.getenv("DSV4_TEST_FORCE_MC2"))
        state["config_printed"] = True
    state["sequence"] += 1
    sequence = state["sequence"]
    fields = dict(**ranks, seq=sequence, local=ctx.num_tokens, selector_tokens=selected_tokens,
                  actual=_plain(actual_tokens), max_dp=ctx.max_tokens_across_dp,
                  pad=ctx.pad_size, padded=getattr(ctx, "padded_num_tokens", None),
                  profile=ctx.in_profile_run, metadata=metadata is not None,
                  draft=ctx.is_draft_model, skip_compiled=skip_compiled,
                  route=getattr(ctx.moe_comm_type, "name", None), capacity=get_mc2_tokens_capacity(),
                  **_metadata(metadata))
        ...
        if profiler is not None:
            ...
            for event in profiler.key_averages():
                ...
                if any(word in name.lower() for word in (
                    "moe_distribute", "moe_dispatch", "dispatch_ffn", "dispatch_gmm",
                    "alltoall", "all_to_all", "allgather", "all_gather", "moe_init_routing",
                    "vllm::moe", "vllm::dsa", "vllm::mla")):
                    counts[name] += event.count
            _emit("OPS", **ranks, seq=sequence, scope="host_may_include_tracing",
                  sample=seen[key], counts=dict(counts))
