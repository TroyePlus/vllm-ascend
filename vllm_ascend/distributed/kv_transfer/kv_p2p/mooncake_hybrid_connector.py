...
class MooncakeAgentMetadata(msgspec.Struct, omit_defaults=True, dict=True):
    ...
    private_circle_layouts: dict[str, dict[str, Any]] | None = None
...
class ReqMeta:
    ...
    private_circle_layout: dict[str, Any] | None = None
...
class KVCacheRecvingThread(threading.Thread):
    def __init__(
        ...
        kv_caches: dict[str, Any],
        private_circle_group_layouts: dict[str, dict[str, Any]],
        ...
        self.private_circle_group_layouts = private_circle_group_layouts
    ...
    def _transfer_match_block_ids(self, local_block_ids: BlockIds) -> list[int]:
        ...
        """
        private_group_ids = {
            int(group_id) for group_id in self.private_circle_group_layouts
        }
    ...
    def _get_remote_metadata(self, remote_host: str, remote_handshake_port: int) -> None:
        ...
        try:
            ...
            if self.private_circle_group_layouts:
                if agent_meta.layout_version != 1:
                    raise RuntimeError(
                        f"unsupported Mooncake private circle layout version: {agent_meta.layout_version}")
                if _canonical_private_layouts(agent_meta.private_circle_layouts) != _canonical_private_layouts(self.private_circle_group_layouts):
                    raise RuntimeError(
                        "Mooncake P/D private circle cache layouts differ: "
                        f"remote={agent_meta.private_circle_layouts}, "
                        f"local={self.private_circle_group_layouts}")
...
class MooncakeConnectorMetadata(KVConnectorMetadata):
    ...
    def add_new_req(
        ...
        self.requests[request_id] = ReqMeta(
            ...
            private_circle_layout=kv_transfer_params.get("private_circle_layout"),
        )
...
class MooncakeConnectorScheduler:
    ...
    def __init__(self, vllm_config: VllmConfig, engine_id: str, kv_cache_config: KVCacheConfig):
        ...
        self._private_circle_receiving_requests: dict[str, Request] = {}
        ...
        self.private_circle_group_ids: set[int] = set()
        self.private_circle_group_layouts: dict[int, dict[str, Any]] = {}
        ...
        for group_id, specs in enumerate(self.kv_cache_specs):
            if not ascend_envs.VLLM_ASCEND_ENABLE_PRIVATE_CIRCLE_POOL:
                continue
            private_spec = next(
                (spec for spec in specs if is_private_circle_kv_cache_spec(spec)), None
            )
            ...
            blocks_per_allocation = ((capacity + block_size - 1)
                                     // block_size)
            self.private_circle_group_ids.add(group_id)
            self.private_circle_group_layouts[group_id] = {
                "layout_version": 1,
                "pool_type": "private",
    ...
    def _private_page_ids(allocation: int, layout: dict[str, Any]) -> list[int]:
        ...
        if allocation < 0 or start < 0 or end < start:
            raise RuntimeError(f"invalid private circle request layout: {layout}")
        if end == start:
            return []
    ...
    def _validate_private_layout(self, layout: dict[str, Any]) -> None:
        ...
        expected = self.private_circle_group_layouts.get(group_id)
        if expected is None:
            raise RuntimeError(f"remote private circle group {group_id} is not configured locally")
        for field, value in expected.items():
            if layout.get(field) != value:
                raise RuntimeError(
                    f"private circle P/D layout mismatch for group {group_id}: "
        ...
        if missing:
            raise RuntimeError(f"private circle P/D metadata missing fields: {missing}")
        ...
        if span < 0 or span > capacity:
            raise RuntimeError(
                "private circle transfer span exceeds one allocation: "
                f"{span} > {capacity}"
            )
        if list(layout["page_order"]) != expected_pages:
            raise RuntimeError("private circle remote page order is inconsistent with its layout")
        if int(layout.get("first_page_offset", -1)) != int(layout["window_start"]) % block_size:
            raise RuntimeError("private circle first partial-page metadata is invalid")
        expected_last = (int(layout["valid_length"]) % block_size or block_size)
        if int(layout.get("last_page_valid_length", -1)) != expected_last:
            raise RuntimeError("private circle last partial-page metadata is invalid")
        ...
        if params is not None and params.get("do_remote_prefill"):
            if params.get("remote_block_ids"):
                if all(p in params for p in ("remote_engine_id", "remote_host", "remote_port", "remote_request_id")):
                    ...
                    private_layout = params.get("private_circle_layout")
                    if self.private_circle_group_ids and num_external_tokens > 0:
                        if private_layout is None:
                            raise RuntimeError("remote DeepSeek-V4 request has no private circle layout")
                        self._validate_private_layout(private_layout)
                        allocation = getattr(request, "private_circle_allocation", None)
                        if allocation is None:
                            raise RuntimeError("decode request has no private circle allocation")
                        ...
                        if (
                            ...
                            raise RuntimeError(
                                "remote private circle does not cover the "
                                f"expected prefix [0, {expected_length})"
                            )
                        request.private_circle_transfer_state = (
                            PRIVATE_CIRCLE_TRANSFER_RECEIVING
                        )
                        request.private_circle_imported_window_start = (
                            imported_start
                        )
                        request.private_circle_imported_valid_length = imported_end
                        transfer_match_block_ids = next(
                            (
                                ...
                                if (
                                    group_id not in self.private_circle_group_ids
                                    and group_blocks
                                )
                            ),
                            None,
                        ...
                        if transfer_match_block_ids is None:
                            raise RuntimeError(
                                "remote private circle transfer has no local "
                                "blocks for failure tracking"
                            )
                        request.private_circle_transfer_shared_block_ids = (
                            frozenset(
                                int(block_id)
                                for block_id in transfer_match_block_ids
                            )
                        )
                        self._private_circle_receiving_requests[
                            request.request_id
                        ] = request
    ...
    def update_connector_output(
        ...
        """Publish private-circle receive completion to scheduler requests."""
        ...
        for request_id in finished_recving:
            request = self._private_circle_receiving_requests.pop(
                request_id, None
            )
            if request is None:
                continue
            ...
            request.private_circle_transfer_state = (
                PRIVATE_CIRCLE_TRANSFER_FAILED
                if transfer_failed
                else PRIVATE_CIRCLE_TRANSFER_READY
            )
    ...
    def request_finished_all_groups(
        ...
        if self.private_circle_group_ids:
            allocation = getattr(request, "private_circle_allocation", None)
            if allocation is None:
                raise RuntimeError("prefill request has no private circle allocation")
            ...
            # group on both sides.
            group_id = min(self.private_circle_group_ids)
            private_layout = dict(self.private_circle_group_layouts[group_id])
            ...
            if valid_length <= 0:
                valid_length = int(getattr(request, "private_circle_valid_length",
            ...
            for private_group_id in self.private_circle_group_ids:
        ...
        return delay_free_blocks, dict(
            ...
            private_circle_layout=private_layout,
        )
...
class MooncakeConnectorWorker:
    ...
    def __init__(self, vllm_config: VllmConfig, engine_id: str, kv_cache_config: KVCacheConfig):
        ...
        self.private_circle_group_layouts: dict[str, dict[str, Any]] = {}
        speculative_config = vllm_config.speculative_config
        draft_tokens = (getattr(speculative_config, "num_speculative_tokens", 0)
                        if speculative_config is not None else 0)
        for group_id, group in enumerate(kv_cache_config.kv_cache_groups):
            ...
            spec = next(
                (item for item in specs if is_private_circle_kv_cache_spec(item)), None
            )
            if spec is not None:
                capacity = int(spec.sliding_window) + draft_tokens
                pages = ((capacity + int(spec.block_size) - 1)
                         // int(spec.block_size))
                self.private_circle_group_layouts[str(group_id)] = {
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        metadata = MooncakeAgentMetadata(
            ...
            private_circle_layouts=self.private_circle_group_layouts,
        )
        ...
        if self.kv_role == "kv_producer":
            ...
            self.kv_recv_thread = KVCacheRecvingThread(
                ...
                self.kv_caches,
                self.private_circle_group_layouts,
