...
def trace_enabled() -> bool:
    return envs_ascend.VLLM_ASCEND_TRACE


def iter_namelist_paths() -> list[Path]:
    paths: list[Path] = []
    custom_path = envs_ascend.PROFILING_NAMELIST
    if custom_path:
        ...
        if path.exists():
            ...
            logger.warning(
                ...
                "exist: %s. Falling back to the packaged namelist.",
                custom_path,
            )
...
def _module_allowed_for_scope(module_name: str, scope: str) -> bool:
    ...
    if scope == "worker":
        return module_name.startswith("vllm_ascend.worker.") or module_name.startswith(
...
def apply_trace_patches(scope: str) -> None:
    ...
    if not paths:
        logger.warning("VLLM_ASCEND_TRACE is enabled but no trace namelist is available.")
