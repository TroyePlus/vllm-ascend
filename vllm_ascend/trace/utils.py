...
ip_str = get_ip()
trace_output_directory = envs_ascend.TRACE_OUTPUT_DIRECTORY


def get_role() -> str:
    return envs_ascend.ROLE
...
def log_action(action: str, request_id: Any, role: str | None = None) -> None:
    ...
    safe_print(
        ...
        f"<<<Action: {action}; Timestamp:{time.time()}; RequestID:{request_id}; Role:{role or get_role()}_{ip_str}",
    )
