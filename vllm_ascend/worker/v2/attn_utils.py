...
def _validate_kv_cache_num_blocks(
    ...
    """Validate shared and private circle cache capacities independently."""
    if (
        envs.VLLM_ASCEND_ENABLE_PRIVATE_CIRCLE_POOL
        and is_private_circle_kv_cache_spec(kv_cache_spec)
    ):
        draft_tokens = (
        ...
        assert num_blocks == private_config.num_blocks, (
            "private circle tensor size does not match its fixed ring layout: "
...
def _view_dsv4_cache(
    ...
    if envs.VLLM_ASCEND_ENABLE_PRIVATE_CIRCLE_POOL and is_private_circle_kv_cache_spec(
        kv_cache_spec
    ):
        _validate_kv_cache_num_blocks(
            num_blocks, kv_cache_config, kv_cache_spec, get_current_vllm_config()
        )
    elif num_blocks != kv_cache_config.num_blocks:
