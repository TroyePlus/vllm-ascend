...
| `mega_moe_max_tokens`               | int  | `65536` | Token capacity for fused MoE operators. A3 passes it to `dispatch_ffn_combine` or uses it as CANN MegaMoE's receive capacity. A5 treats it as a global token ceiling and allocates a symmetric buffer for `min(mega_moe_max_tokens // expert_parallel_size, execution_tokens_per_rank)` tokens per rank. Workspace memory scales linearly with the allocated capacity. |
